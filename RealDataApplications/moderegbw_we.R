"moderegbw.w" <- function(Y, X, method="CV-density", p.order=0, h1=NULL, h2=NULL, nstart = 4,weight=rep(1,length(X)),
                                                   xinterval = quantile(X, probs=c(0.025, 0.975), names = FALSE),
                                                   df=5, ncomp=5, nboot=20){                           
  #########################################################################################
  # data structure
  #########################################################################################
  n = length(X);
  ...
  #    CV = .Call("CVmode_LCfit", dd[,1], dd[,2], dd[,3], meshy, yindx, h1, h2, 200, 1e-4, PACKAGE = "lpme")$CV; 
      CV = CVmode_LCfit1( dd[,1], dd[,2], dd[,3], meshy, yindx, h1, h2, weight,200, 1e-4)$CV; 
 ...
 #       fit = modereg(newY, X, mesh=mesh, bw=band.h[j,], p.order=p.order)$mode;
        fit = modereg.w(newY, X,weight=weight, mesh=mesh, bw=band.h[j,], p.order=p.order)$mode;
        dis = rep(0, nx);
        for(i in 1:nx){
...
#        for(ii in 1:length(ym)) tmp[ii] = min(abs(ymhat-ym[ii]));  
          for(ii in 1:length(ym)) tmp[ii] = min(abs(ymhat-ym[ii])*(1/weight));
          dis1 = max(tmp);
          tmp = rep(0, nyx);
