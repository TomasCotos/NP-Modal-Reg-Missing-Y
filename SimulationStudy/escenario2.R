...
simulacion<-function(nr,nsamp,tipo.mis,a1,a2,b1,b2,p1){
  ARCHIVOSALIDA<-paste("BI","_M",tipo.mis,"a1a2",a1,a2,p1,"p.txt",sep="")  
  ...
  for (l in 1:nr){
    ...
    y<-m1(x)+eps
    #eps es una mixtura de normales de media y moda  a1 y a2. 
    # por tanto la moda viene dada por m1(x)+a1 y m1(x)+a2
...
### repite a función ata que non da erro!!!
    ...
    for(i in 1:length(xmis)){
      ...
      for (i1 in 1:length(modas)){
        pyx[i1]=conditional_density(x0[i1],y0=modas[i1],X=xcomp,
                               Y=ycomp,w=rep(1,length(xcomp)),bwx=bw$a,bwy=bw$b) 
        }
    ...
    for(nimput in 1:3){
      ...
      yhat=dat[,2]
      n1=length(xhat)
      hhxy_i = moderegbw.w(yhat, xhat, method=vent, p.order=0,
                           h1=NULL, h2=NULL,weight=rep(1,n1),)$bw
