...
simulacion<-function(nr,nsamp,tipo.mis,a1,a2,b1,b2,p1){ 
  ...
  for (l in 1:nr){
    ...
    print(l)
    x<-sort(runif(n))
#      ESCENARIO 1 -- ERR(Y|X=x)=ERR(Y)
     eps <- error.dep.x(x=rep(0,n),mu=c(a1,a2), sig=c(b1,b2),w=c(p1,1-p1))
    y<-m1(x)+eps
    ...
    fit_c=modereg(y, W=x, xgrid=xgrid, bw=hhxy, nstart=4,sig=NULL, p.order=0,
                  PLOT=FALSE)
    ...
    # Estimación de la regresión modal. Suponemos nstar=4  
    fit_s=modereg.w(Y=ycomp, W=xcomp, xgrid=xgrid, bw=hhxy_s, nstart=4,sig=NULL,
                     p.order=0, weight=rep(1,length(ycomp)),PLOT=FALSE)
    ## Estimación Ponderada
    # Estimación del parámetro por validación cruzada o bootstrap  Zhou y Huang(2019)
    hhxy = moderegbw.w(Y=ycomp, X=xcomp, method=vent, p.order=0,h1=NULL, 
                       h2=NULL, weight=1/pycomp)$bw
    # Estimación de la regresión modal. Suponemos nstar=4  
    fit_w=modereg.w(Y=ycomp, W=xcomp, xgrid=xgrid, bw=hhxy, nstart=4,
                    sig=NULL, p.order=0, weight=1/pycomp, PLOT=FALSE)
    ## Estimación Imputada
    fit_prev=modereg.w(ycomp, W=xcomp, xgrid=xmis, bw=hhxy_s, nstart=4,sig=NULL,
                        p.order=0, weight=rep(1,length(ycomp)),PLOT=FALSE)
    
    ### IMPUTACIÓN
    ###########################
    # Estimación de la regresión modal. Suponemos nstar=4 
    # Imputar por un solo valor, el más probable.
...
### repite a función ata que non da erro!!!
  ...
  #      pyx[i1]=conditional_density(x0[i1],y0=modas[i1],X=xcomp,Y=ycomp,w=rep(1,length(xcomp)),bwx=hhxy[1],bwy=hhxy[2]) 
        pyx[i1]=conditional_density(x0[i1],y0=modas[i1],X=xcomp,Y=ycomp,w=rep(1,length(xcomp)),bwx=bw$a,bwy=bw$b) 
        }
    ...
    yhat=dat[,2]
    n1=length(xhat)
    hhxy = moderegbw.w(yhat, xhat, method=vent, p.order=0,h1=NULL, h2=NULL,weight=rep(1,n1),)$bw
    fit_imput1=modereg.w(yhat, W=xhat, xgrid=xgrid, bw=hhxy, nstart=4,sig=NULL, p.order=0, weight=rep(1,n1),PLOT=FALSE)
    # IMPUTACIÓN MÚLTIPLE
    # Supongamos nstar=4
